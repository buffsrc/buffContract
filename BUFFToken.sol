...
contract BUFFToken is ERC20, Ownable {
    ...
    event SuperLotteDrawn(address indexed winner, uint256 amount, uint256 timestamp);
    ...
    uint256 public gov2Fee = 100;
    uint256 public gov3Fee = 100;

    uint256 public luckDropFee = 400;
    uint256 public bonusFee = 400;
    uint256 public superLottoFee = 400;
    uint256 public distributeEthFee = 1200;
    ...
    // Fees receivers
    address public gov1Wallet = address(0x1);
    address public gov2Wallet = address(0x2);
    address public gov3Wallet = address(0x3);
    
    address private bonusWallet;
    address public lastTradeAddress;
    IJackpot private luckDropWallet;

    uint256 public launchedAt;
    // uint256 public launchedAtTimestamp;
    uint256 public superLottoAmount;
    uint256 public lastTradeTime;
    uint256 public tradeCount;
    uint256 public paiedLottoAmount;
    uint256 public paiedLottoCount;

    uint256 public superLottoCooldown = 24 hours;
    ...
    function _dogTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if ((isPair(sender) || isPair(recipient)) && !isFeeExempt[sender]) {
            require(launched(), "Trading not open yet");
        }

        if (lastTradeTime + superLottoCooldown < block.timestamp && superLottoAmount > 0){
            uint needPay = superLottoAmount;
            superLottoAmount = 0;
            lastTradeTime = block.timestamp;
            payable(lastTradeAddress).transfer(needPay);
            paiedLottoAmount = paiedLottoAmount + needPay;
            paiedLottoCount = paiedLottoCount + 1;
            tradeCount = 0;
            emit SuperLotteDrawn(lastTradeAddress, needPay, block.timestamp);
        }
        ...
        if (isPair(sender)) {
            ...
            side = 1;
            lastTradeTime = block.timestamp;
            lastTradeAddress = recipient;
            tradeCount = tradeCount + 1;
            try luckDropWallet.tradeEvent(sender, amount) {} catch {}
        ...
        if (!isPair(sender) && !isPair(recipient) && shouldSwapBack()) {
            swapBack();
        }
    ...
    function swapBack() internal swapping {
        ...
        //     success = true;
        // } catch {
        try swapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(taxAmount,0,path,address(this),block.timestamp + 300) {
            success = true;
        } 
        catch {}
        // }
        ...
        uint256 gotten = address(this).balance - balanceBefore;
       uint amountBonus = bonusFee * gotten / distributeEthFee;
       uint amountLuckDrop = luckDropFee * gotten / distributeEthFee;
       uint amountLotto = gotten - amountBonus - amountLuckDrop;
       superLottoAmount = superLottoAmount + amountLotto;

       payable(address(luckDropWallet)).transfer(amountLuckDrop);
        payable(address(bonusWallet)).transfer(amountBonus);

//TODO: send bonus + luckdrop reward and transfer
    ...
    function launched() internal view returns (bool) {
        return block.timestamp >= launchedAt && launchedAt != 0;
    }


    function takeFee(address sender, uint256 amount) internal returns (uint256 restAmount) {
        uint256 totalFees = amount * totalFee / feeDenominator;
        restAmount = amount - totalFees;
        _transfer(sender, address(this), totalFees);
        _transfer(address(this), gov1Wallet, amount * gov1Fee / feeDenominator);
        _transfer(address(this), gov2Wallet, amount * gov2Fee / feeDenominator);
        _transfer(address(this), gov3Wallet, amount * gov3Fee / feeDenominator);
    }
    ...
    /*** ADMIN FUNCTIONS ***/
    function setLaunch(uint _launch) public onlyOwner {
        launchedAt = _launch;
    }
    ...
    function setFeeReceivers(
        ...
        gov3Wallet = _gov3Wallet;
        bonusWallet = _bonusWallet;
        luckDropWallet = IJackpot(_jackpotWallet);
    }
    function getBUFFFromETH(uint amount) public view returns(uint) {
        if (amount == 0) {
            return 0;
        }
        IUniswapV2Factory _factory = IUniswapV2Factory(swapRouter.factory());

        address _pairAddress = _factory.getPair(address(WETH), address(this));
        ...
        if (_pair.token0() == address(this)) {
            return swapRouter.quote(amount, reserve1, reserve0);
        }else if (_pair.token0() == address(WETH)) {
            return swapRouter.quote(amount, reserve0, reserve1);
        }
        return 0;
    } 

    function getETHFromBUFF(uint amount) public view returns(uint) {
        if (amount == 0) {
            return 0;
        }
        IUniswapV2Factory _factory = IUniswapV2Factory(swapRouter.factory());
        ...
        uint numerator = reserve0 *(amount) *(10000);
        uint denominator = (reserve1 - (amount)) * (9975);
        return (numerator / denominator) + 1e9;

        }else if (_pair.token1() == address(WETH)) {
            uint numerator = reserve1 *(amount) *(10000);
            uint denominator = (reserve0 - (amount)) * (9975);
            return (numerator / denominator) + 1e9;
        }
        return 0;
    } 
